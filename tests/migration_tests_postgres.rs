...
async fn postgres_migration_discovery() {
    ...
    let tables: Vec<neutrino_schema::introspect::TableInfo> = introspector
    ...
    for name in &[
        ...
        assert!(
            tables.iter().any(|ti| ti.name == *name),
            "missing table: {name}"
        );
    }
    drop(introspector);
...
async fn postgres_migration_types() {
    ...
    assert_eq!(
        is_active.ty,
        DbType::SmallInt,
        "is_active SMALLINT -> DbType::SmallInt"
    );
    ...
    assert_eq!(
        last_login_ip.ty,
        DbType::Inet,
        "last_login_ip INET -> DbType::Inet"
    );
    ...
    assert_eq!(
        created_at.ty,
        DbType::TimestampTz,
        "created_at TIMESTAMPTZ -> DbType::TimestampTz"
    );
    ...
    assert_eq!(
        deleted_at.ty,
        DbType::TimestampTz,
        "deleted_at TIMESTAMPTZ -> DbType::TimestampTz"
    );
    ...
    let token = tokens
        ...
        .iter()
        .find(|f| f.name == "token")
        .expect("oauth_refresh_tokens.token field not found");
    ...
    let revoked = tokens
        .fields
        .iter()
        .find(|f| f.name == "revoked")
        .expect("oauth_refresh_tokens.revoked field not found");
    assert_eq!(
        revoked.ty,
        DbType::Boolean,
        "revoked BOOLEAN -> DbType::Boolean"
    );
